...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private static final int MSG_SIMLOCK_UPDATE = 337;
    private static final int MSG_UPDATE_FACELOCK_STATE = 338;
    ...
    //SPRD: add for bug 765121
    private boolean mAuthentionSucceed;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_TIME_TICK.equals(action)
                ...
                mHandler.obtainMessage(MSG_SIM_STATE_CHANGE, args.subId, args.slotId, args.simState)
                        .sendToTarget();
                if (KeyguardSupportSimlock.getInstance(mContext).isSimlockStatusChange(intent)) {
    ...
    private void handleFaceLockError(int msgId, String errString) {
        ...
        if(msgId == FACE_ERROR_LOCKOUT) {
            int secondsRemaining = mFaceManager != null ? mFaceManager.getLockoutLeftTime() : 0;
            if(secondsRemaining != 0) {
                errString = mContext.getString(R.string.kg_too_many_failed_attempts_countdown, secondsRemaining);
            }
        }
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
    ...
    private static class SimData {
        ...
        public String toString() {
            return "SimData{state=" + simState + ",slotId="
                    + slotId + ",subId=" + subId + ",isUnlock=" + isUnlock + "}";
        }
    }
    ...
    private boolean shouldListenForFaceLock() {
        return (mKeyguardIsVisible || !mDeviceInteractive) && !mBouncer
                 && !mGoingToSleepDisableFaceLock && !mSwitchingUser && mBootCompleted;
    }
    ...
    private boolean isFaceLockDisabled(int userId) {
        ...
        boolean settingsEnable = Settings.System.getInt(mContext.getContentResolver(),
                FACEID_FUNCTION, 0) == 1;
        Log.d(TAG, "currentUser.isOwner()" + currentUser.isOwner());
        if(!currentUser.isOwner()) {
            try{
                Context context = mContext.createPackageContextAsUser(mContext.getPackageName(), 0, currentUser);
                settingsEnable = Settings.System.getInt(context.getContentResolver(),
                        FACEID_FUNCTION, 0) == 1;
        ...
        return mFaceManager != null && !mFaceManager.isHardwareDetected()
                ...
                || enrolledFaces <= 0 || Settings.System.getInt(mContext.getContentResolver(),
                        FACEID_FUNCTION, 0) == 0;
    }

    private void startListeningForFaceLock() {
        if (mFaceLockRunningState == FACELOCK_STATE_CANCELLING) {
            setFingerprintRunningState(FACELOCK_STATE_CANCELLING_RESTARTING);
            return;
        }
    ...
    private void updateFingerprintListeningState() {
        ...
        /* SPRD: add for bug 834599 @{ */
        if(shouldListenForFingerprint && !mKeyguardIsVisible && !mKeyguardOccluded) {
            mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_FINGERPRINT_AUTHENTICATION_CONTINUE),
                    FINGERPRINT_CONTINUE_DELAY_MS);
            return;
        }
    ...
    private boolean shouldListenForFingerprint() {
        //SPRD: fix for bug 765121
        return (mKeyguardIsVisible || !mDeviceInteractive ||
                (mBouncer && !mKeyguardGoingAway) || mGoingToSleep ||
                shouldListenForFingerprintAssistant() || (mKeyguardOccluded && mIsDreaming))
                && !mSwitchingUser && !isFingerprintDisabled(getCurrentUser())
                && !mKeyguardGoingAway && !mAuthentionSucceed;
    }
    ...
    private void startListeningForFingerprint() {
        ...
        if(!mLockPatternUtils.isSecure(userId)) {
            Log.d(TAG, "current not secure mode, cannot use fingerprint");
            return;
        }
    ...
    private void handleSimStateChange(int subId, int slotId, State state) {
        ...
        /* SPRD: modify by BUG 764097 @{ */
        if (changed && slotId > 0 && state == IccCardConstants.State.PIN_REQUIRED) {
            for (int i = 0; i < slotId; i++) {
                int[] subIdValue = SubscriptionManager.getSubId(i);
                if (subIdValue != null && subIdValue[0] > 0) {
                    refreshSimState(subIdValue[0], i);
                }
            }
        }
        /* @} */
    ...
    private boolean resolveNeedsSlowUnlockTransition() {
        ...
        /* SPRD: 730277 {@ */
        if (mContext.checkSelfPermission(Manifest.permission.SET_PREFERRED_APPLICATIONS) == PackageManager.PERMISSION_GRANTED) {
    ...
    private boolean refreshSimState(int subId, int slotId) {
        ...
        if (data == null) {
            ...
            /* SPRD: add for bug 712672 & 764070 @{ */
            boolean isUnlock = data.isUnlock;
            Log.d(TAG, "isUnlock = " + isUnlock + "; subId : " + subId);
            if (isUnlock) {
                return false;
            }
            /* @} */
