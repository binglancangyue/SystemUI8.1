...
public class StorageNotification extends SystemUI {
    ...
    /* SPRD bug 709369:Change notification after change language.{@*/
    ...
    public void start() {
        ...
        /*SPRD bug 708333:Show SDCard mounted notication in owner user only.{@*/
    ...
    private Notification onVolumeMounted(VolumeInfo vol) {
        ...
        /*SPRD Bug 708333:Uninstall SD/OTG,then install SD/OTG,not show notification when from A user switchs to B user.@{*/
        if (vol.getMountUserId() != mCurrentUserId) {
            return null;
        }
        /*@}*/
    ...
    private PendingIntent buildUnmountPendingIntent(VolumeInfo vol) {
        if (ActivityManager.isUserAMonkey()) {
            Log.d(TAG, "monkey test is running, invalid action!");
            return null;
        }
        Log.d(TAG, "get PendingIntent to unmount sd card!");
        final Intent intent = new Intent();
        if (isTv()) {
            intent.setPackage("com.android.tv.settings");
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        try {
             ...
             if(newConfig != null &&  !newConfig.locale.equals(mLocale)){
                ...
                final List<VolumeInfo> vols = mStorageManager.getVolumes();
                for (VolumeInfo vol : vols) {
                    /*SPRD bug 708333:Not show SD or OTG notification.{@*/
                    if (vol.getMountUserId() == mCurrentUserId) {
                        onVolumeStateChangedInternal(vol);
                    }
                }
    ...
    private final BroadcastReceiver mConfigrationReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                mNotificationManager.cancelAsUser(null, SystemMessage.NOTE_STORAGE_PUBLIC,
                        UserHandle.ALL);
                mNotificationManager.cancelAsUser(null, SystemMessage.NOTE_STORAGE_MOVE,
                        UserHandle.ALL);
                ...
                for (VolumeInfo vol : vols) {
                    ...
                    if (vol.getMountUserId() == mCurrentUserId) {
                        ...
                        Log.d(TAG, "onReceive cancelAsUser vol.getId()="+vol.getId());
                        mNotificationManager.cancelAsUser(vol.getId(), SystemMessage.NOTE_STORAGE_PUBLIC,
                                UserHandle.ALL);
                    }
                    /*@}*/
