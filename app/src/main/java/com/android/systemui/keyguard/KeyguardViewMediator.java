...
public class KeyguardViewMediator extends SystemUI {
    ...
    /*
     * SPRD: Bug 660856 the state indicate that decrypt the encrypted phone and trigger
     * restart framework @{
     */
    private static final String DECRYPT_STATE = "trigger_restart_framework";
    private static final String VOLD_DECRYPT = "vold.decrypt";
    /* @} */
    ...
    /**
     * bug713737
     * we send this intent when the keyguard is shown.
     */
    private static final Intent USER_UNPRESENT_INTENT = new Intent("android.intent.action.USER_UNPRESENT")
            .addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING
                    | Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
    ...
    private boolean mSubsidySecure = true;
    KeyguardUpdateMonitorCallback mUpdateCallback = new KeyguardUpdateMonitorCallback() {
                    ...
                    synchronized (KeyguardViewMediator.this) {
                        ...
                        if (simState == ABSENT) {
                            ...
                            // so we should only lock when they are ABSENT.
                             /*
                             *  SPRD: Bug 844163 screen got locked automatically after disable any sim card from sim settings
                             */
                            if(TelephonyManagerEx.from(mContext).isSimEnabled(slotId)){
                ...
                case PUK_REQUIRED:
                    synchronized (KeyguardViewMediator.this) {
                        if (!mShowing) {
                            ...
                            /*
                             *  SPRD: Bug 660856 phone is the encryption process, don't display
                             *  pin or puk keygurad @{
                             */
                            String decryptState = SystemProperties.get(VOLD_DECRYPT);
                            Log.d(TAG, "decrypt state --> " + decryptState);
                            if (!TextUtils.isEmpty(decryptState)
                                    && !DECRYPT_STATE.equals(decryptState)) {
                                return;
                            }
                            /* @} */
                            doKeyguardLocked(null);
        ...
        private void onSimAbsentLocked() {
	     if (DEBUG_SIM_STATES) Log.v(TAG, "mLockWhenSimRemoved: " + mLockWhenSimRemoved + "mShowing: "+ mShowing);
        ...
        public void onFaceLockAuthenticated() {
            ...
            if(maybeCameraActivity()) {
                if (DEBUG) Log.d(TAG, "keyguardDoned delay..");
    ...
    private SpannableString getClickableSpan() {
        View.OnClickListener l = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Uri uri = Uri.parse("http://pqta.telcel.com");
    ...
    public boolean isSecure(int userId) {
        return mLockPatternUtils.isSecure(userId)
                ...
                || KeyguardSubsidyLockController.getInstance(mContext).isSubsidyLock();
    }
    ...
    /* SPRD:Add for bug713737 androido porting usb security checking @{ */
    private void sendUserUnPresentBroadcast() {
        synchronized (this) {
            if (mBootCompleted) {
                ...
                for (UserInfo ui : userHandles) {
                    mContext.sendBroadcastAsUser(USER_UNPRESENT_INTENT, ui.getUserHandle());
                }
            }
        }
    }
    ...
    public void onBootCompleted() {
        ...
        Log.d(TAG, "DEVICE_PROVISIONED = " + Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.DEVICE_PROVISIONED, 0)+ "; mShowing="+mShowing);
        if (!mUpdateMonitor.isDeviceProvisioned() && mShowing == false) {
            mShowing = true;
            adjustStatusBarLocked();
            mShowing = false;
            adjustStatusBarLocked();
