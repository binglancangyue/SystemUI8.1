...
public class StatusBarIconView extends AnimatedImageView {
    ...
    public StatusBarIconView(Context context, String slot, StatusBarNotification sbn,
        ...
        if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, "get shouldRefresh in new StatusBarIconView()_1, statusbarVisible:" + shouldRefresh);
    ...
    public StatusBarIconView(Context context, AttributeSet attrs) {
        super(context, attrs);
        shouldRefresh = StatusBar.getStatusBarVisibility();
        if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, "get shouldRefresh in new StatusBarIconView()_2, statusbarVisible:" + shouldRefresh);
        mDozer = new NotificationIconDozeHelper(context);
        mBlocked = false;
    ...
    public void invalidate() { 
        if (!shouldRefresh && StatusBar.ALLOW_NOT_REFRESH) {
            Log.d(TAG_NOT_REFRESH, "StatusBarIconView no necessary to invalidate");
            return;
        }
        super.invalidate();
    }
    ...
    private BroadcastReceiver mUpdateStatuaBarVisibleReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context ctx, Intent intent) {
            ...
            if (shouldRefresh != statusBarVisible) {
                if (statusBarVisible) updateIconColor();
    ...
    private void updateIconColor() {
        if (!shouldRefresh && StatusBar.ALLOW_NOT_REFRESH) {
            if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, "StatusBarInvisible, no necessary to update statusBarIcon");
            return;
        }
        /* @} SPRD bug 814301&819675 :add control of whether refresh statusbar icons*/
