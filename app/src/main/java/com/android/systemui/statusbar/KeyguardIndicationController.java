...
public class KeyguardIndicationController {
    ...
    private BroadcastReceiver mUpdateStatuaBarVisibleReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context ctx, Intent intent) {
            ...
            if (shouldRefresh != statusBarVisible) {
                if (statusBarVisible) updateIndication();
    ...
    protected final void updateIndication() {
        ...
        if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, "updateIndication：KeyguardIndication　Visible ? " +mVisible);
        if (mVisible) {
            if (!shouldRefresh && StatusBar.ALLOW_NOT_REFRESH) {
                if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, "should not updateIndication as statusbar invisible ?");
                return;
            }
            /* @} SPRD bug 814301&819675 :add control of whether refresh statusbar icons*/
