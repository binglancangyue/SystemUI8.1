...
/* SPRD: Bug698953 left slide into AudioProfile @{ */
...
public class KeyguardBottomAreaView extends FrameLayout implements View.OnClickListener,
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        if (!SystemuiFeatureUtil.getInstance().isCMCC()) {
            mLeftAffordanceView.setImageDrawable(mContext.getDrawable(R.drawable.ic_camera_alt_24dp));
        }
        /* @} */
    ...
    private void updateRightAffordanceIcon() {
        ...
        boolean isSuperMode = (mStatusBar != null) && (mStatusBar.getPowerSaveModeInternal() == PowerManagerEx.MODE_ULTRASAVING);
        mRightAffordanceView.setVisibility(!mDozing && state.isVisible
            && !isSuperMode ? View.VISIBLE : View.GONE);
    ...
    private Intent getCameraIntent() {
	 if(mSource == CAMERA_LAUNCH_SOURCE_POWER_DOUBLE_TAP)
        {
            KeyguardUpdateMonitor updateMonitor = KeyguardUpdateMonitor.getInstance(mContext);
    ...
    public void launchCamera(String source) {
        Log.d(TAG,"launchCamera source ="+source);
    ...
    public void applyKeyguardTextColor(int color) {
        ...
        mEnterpriseDisclosure.setTextColor(color);
        updateIconColor(color);
    }

    private void updateIconColor(int color) {
        mRightAffordanceView.updateIconColor(color);
        mLeftAffordanceView.updateIconColor(color);
        mLockIcon.updateIconColor(color);
    }

    public void updateIconColor(boolean left, boolean dark) {
        if (left) {
            mLeftAffordanceView.updateIconColor(dark ? Color.BLACK : mColor);
        } else {
            mRightAffordanceView.updateIconColor(dark ? Color.BLACK : mColor);
        }
    }
    ...
    public void onUnlockMethodStateChanged() {
        ...
        mLockIcon.update(isFaceLockEnable());
        updateCameraVisibility();
    }
    ...
    private void inflateCameraPreview() {
	 ...
	 if (!SystemuiFeatureUtil.getInstance().isCMCC() || (mSource == CAMERA_LAUNCH_SOURCE_POWER_DOUBLE_TAP)){
    ...
    private void setRightButton(IntentButton button) {
        ...
        inflateCameraPreview();
        /* SPRD: Bug698953 left slide into AudioProfile @{ */
        SystemuiFeatureUtil.getInstance().changeCameraToProfile(mRightAffordanceView);
        /* @} */
    }
    ...
    public void setDozing(boolean dozing, boolean animate) {
        ...
        if (dozing) {
            ...
            boolean isSuperMode = (mStatusBar != null) && (mStatusBar.getPowerSaveModeInternal() == PowerManagerEx.MODE_ULTRASAVING);
            boolean isFingerprint =mLockIcon.isFingerprinterLock();
            mLockIcon.setVisibility( isSuperMode && !isFingerprint ? INVISIBLE : VISIBLE);
    ...
    private class DefaultRightButton implements IntentButton {
        ...
        @Override
        public IconState getIcon() {
            //SPRD: Bug 853393 set default icon state
            if (SystemuiFeatureUtil.getInstance().isCMCC()) {
                mIconState.isVisible = mUserSetupComplete;
                mIconState.drawable = mContext.getDrawable(R.drawable.ic_shock_alt_24dp);
                mIconState.contentDescription =
                        mContext.getString(R.string.vibrate_hint);
                return mIconState;
            }
