...
public class NotificationChildrenContainer extends ViewGroup {
    ...
    private BroadcastReceiver mUpdateStatuaBarVisibleReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context ctx, Intent intent) {
            ...
            if (shouldRefresh != statusBarVisible) {
                if (statusBarVisible) setDark(mDark, mFade, mDelay);
    ...
    public void setDark(boolean dark, boolean fade, long delay) {
        if (!shouldRefresh && StatusBar.ALLOW_NOT_REFRESH) {
            if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, "NotificationChildrenContainer no necessary to setDark");
            mDark = dark;
            mFade = fade;
            mDelay = delay;
            return;
