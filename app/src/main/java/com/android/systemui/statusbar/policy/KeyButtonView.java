    ...
    private final Runnable mCheckLongPress = new Runnable() {
        public void run() {
            if (isPressed()) {
                ...
                /* SPRD: fix for bug 848172 @{ */
                if ("com.sprd.validationtools".equals(getTopActivityPackageName(mContext))
                        && KeyButtonView.this.getId()== R.id.recent_apps){
                    mCode = 999;
                }else if(KeyButtonView.this.getId()== R.id.recent_apps){
                    mCode = 0;
                }
                /* @} */
                if (isLongClickable()) {
