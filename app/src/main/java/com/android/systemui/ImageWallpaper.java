...
public class ImageWallpaper extends WallpaperService {
    ...
    class DrawableEngine extends Engine {
        ...
        private AsyncTask<Void, Void, Bitmap> mUnlockWallpaper;
        private boolean mNeedsDrawAfterLoadingWallpaper;
        private boolean mSurfaceValid;

        /*SPRD bug 860764:Flash black screen while bitmap not loaded yet.*/
        int mTempRotation = -1;
        ...
        void drawFrame() {
            ...
            try {
                ...
                /*SPRD bug 860764:Flash black screen while bitmap not loaded yet.*/
                if(mTempRotation == -1 && !(mBackground.getWidth() == dw && mBackground.getHeight() == dh) && mIsHwAccelerated){
                    if(dw == displayInfo.logicalWidth && dh == displayInfo.logicalHeight
                            && mBackground.getWidth() >= dw && mBackground.getHeight() >= dh){
                        Log.w(TAG, "Bitmap not loaded yet,just return!");
                        return;
                    }
                }
                mTempRotation = newRotation;
                /*@}*/
                //SPRD: fix for bug 858317
                ...
                if (DEBUG) {
                    Log.d(TAG, "Redrawing wallpaper mIsHwAccelerated="+mIsHwAccelerated+",mBackground.getWidth()"+mBackground.getWidth()
                            +",mBackground.getHeight()="+mBackground.getHeight());
                    Log.d(TAG, "Redrawing wallpaper width="+sh.getSurfaceFrame().width()+",sh.getSurfaceFrame().height()="+sh.getSurfaceFrame().height());
                    Log.d(TAG, "Redrawing wallpaper logicalWidth="+displayInfo.logicalWidth +",logicalHeight="+displayInfo.logicalHeight);
                    Log.d(TAG, "Redrawing wallpaper availw="+availw+",availh="+availh+",xPixels="+xPixels+",yPixels="+yPixels);
                }
                if (mIsHwAccelerated) {
        ...
        private void unloadWallpaper(boolean forgetSize) {
            ...
            if (mUnlockWallpaper != null) {
                mUnlockWallpaper.cancel(false);
                mUnlockWallpaper = null;
            }
            mUnlockWallpaper = new AsyncTask<Void, Void, Bitmap>() {
                @Override
                protected Bitmap doInBackground(Void... params) {
                    mWallpaperManager.forgetLoadedWallpaper();
                    return null;
