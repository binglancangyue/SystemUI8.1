...
public abstract class QSTileImpl<TState extends State> implements QSTile {
    ...
    protected QSTileImpl(QSHost host) {
        ...
        if (StatusBar.ALLOW_NOT_REFRESH)
            mContext.registerReceiver(mUpdateStatuaBarVisibleReceiver, new IntentFilter("statusbar visibility changed"));
        if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, this.getClass().getSimpleName() + " registerReceiver");
    }
    ...
    public void destroy() {
        if (StatusBar.ALLOW_NOT_REFRESH)
            mContext.unregisterReceiver(mUpdateStatuaBarVisibleReceiver);
    ...
    protected void handleLongClick() {
        // SPRD: add for bug721525
        isNeedCollapsePanels();
        Dependency.get(ActivityStarter.class).postStartActivityDismissingKeyguard(
                getLongClickIntent(), 0);
    }

    /*SPRD: add for bug721525 @{ */
    protected void isNeedCollapsePanels() {
        String tileSpec = getTileSpec();
        Log.d(TAG, " isNeedCollapsePanels = " + tileSpec);
        if (DATA.equals(tileSpec) || SAVER.equals(tileSpec) || AIRPLANE.equals(tileSpec)) {
    ...
    private BroadcastReceiver mUpdateStatuaBarVisibleReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context ctx, Intent intent) {
            ...
            if (shouldRefresh != statusBarVisible) {
                if (statusBarVisible) handleStateChanged();
                shouldRefresh = statusBarVisible;
            }
            if (DEBUG_NOT_REFRESH) android.util.Log.d(TAG_NOT_REFRESH, "Clock-onSystemUiVisibilityChange: statusBarVisible = " + statusBarVisible);
        }
    };
    ...
    protected void handleRefreshState(Object arg) {
        ...
        if (changed) {
            if (!shouldRefresh && StatusBar.ALLOW_NOT_REFRESH) {
                if (DEBUG_NOT_REFRESH) Log.d(TAG_NOT_REFRESH, this.getClass().getSimpleName() + " should not refresh since statusbar invisible");
                return;
                /* @} SPRD bug 814301&819675 :add control of whether refresh statusbar icons*/
            }
            handleStateChanged();
